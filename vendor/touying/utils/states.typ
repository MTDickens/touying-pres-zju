...
#let touying-progress(callback) = context {
  ...
  if last-slide-counter.final().first() == 0 {
    ...
    let ratio = calc.min(1.0, slide-counter.at(loc).first() / last-slide-counter.final().first())
    callback(ratio)
  }
}
...
#let touying-final-sections(callback) = context callback(sections-state.final())
