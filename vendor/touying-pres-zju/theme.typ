...
#let zjublue=rgb("#034ea1")


 //横向导航栏
#let zju-nav-bar(self: none) = states.touying-progress-with-sections(dict => {
 ...
 //章节目录页（）
#let zju-outline(self: none) = states.touying-progress-with-sections(dict => {
  let (current-sections, final-sections) = dict
  current-sections = current-sections.filter(section => section.loc != none).map(section => (
    section,
    section.children,
  ...
  for (i, section) in final-sections.enumerate() {
    ...
    if  i == current-index {
      block(height: 2pt, width: 100%, spacing: 0pt, utils.call-or-display(self, self.zju-chapter-progress-bar))
    }
  }
...
#let slide(
  ...
  if title != auto {
    self.zju-title = title
  }
  if subtitle != auto {
    self.zju-subtitle = subtitle
  }
  if header != auto {
    self.zju-header = header
  }
  if footer != auto {
    self.zju-footer = footer
  }
  if display-current-section != auto {
    self.zju-display-current-section = display-current-section
  }
...
#let title-slide(self: none, ..args) = {
  ...
  let content = {
    ...
    // place(right + top, image("assets/img/zju_logo_side.svg",format: "svg"), dx: -1em)
  }
...
#let outline-slide(self: none) = {
  ...
  self.zju-title = context if text.lang == "zh" [大纲] else [Outline]
  //   set page(background: rotate(-60deg,
  //   place(right + horizon, image("assets/img/zju_logo_side.svg",format: "svg", width: 25%), dx: 14em,dy:18em)
  // ),
  // )
  let content = {
    set align(horizon)
...
#let register(
  ...
  let githublink= if self.info.github != "" {
    ...
    "https://github.com/MTDickens/touying-pres-zju"
  }
  ...
  self.zju-enable-progress-bar = progress-bar

  self.zju-progress-bar = self => states.touying-progress(ratio => {    
    grid(
  ...
  self.zju-chapter-progress-bar = self => states.touying-progress(ratio => {    
    grid(
      columns: (ratio * 100%, 1fr),
      rows: 2pt,
  ...
  self.zju-navigation = self => {
    grid(
      align: center + horizon,
      columns: (1fr, auto, auto),
      rows: 1em,
      components.cell(fill: self.colors.primary, zju-nav-bar(self: self)),
      // block(fill: self.colors.neutral-lightest, inset: 4pt, height: 100%, image("assets/img/zju_logo_side.svg")),
    )
  }

  self.zju-title = none
  self.zju-subtitle = none
  self.zju-footer = self => {
    ...
    grid(
      ...
      cell(fill: self.colors.primary, utils.call-or-display(self, footer-c)),
      cell(fill: self.colors.primary, zju-nav-bar(self: self)),
      // cell(fill: self.colors.primary, utils.call-or-display(self, footer-a)),
  ...
  self.zju-header = self => {
    if self.zju-title != none {
      block(
        width: 100%,
        height: 2em,
      ...
      place(left + horizon, text(fill: self.colors.neutral-lightest, weight: "bold", size: 1.3em, self.zju-title), dx: 1.5em)
  ...
  let header(self) = {
    ...
    grid(
      rows: (auto, auto),
      // utils.call-or-display(self, self.zju-navigation),
      utils.call-or-display(self, self.zju-header),
    )
  }
  let footer(self) = {
    set text(size: .5em)
    ...
    grid(
      rows: (auto, auto),
      utils.call-or-display(self, self.zju-footer),
      if self.zju-enable-progress-bar {
        utils.call-or-display(self, self.zju-progress-bar)
      },
    )
  }
  ...
  self.methods.init = (self: none, lang: "zh", font: ("Linux Libertine",), body) => {
    ...
    set list(marker: self.zju-knob-marker)
