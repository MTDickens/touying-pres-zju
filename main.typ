...
#slide(composer: (1.2fr, 1.38fr))[
  #text(0.7em)[
    ...
    - *权重训练*
      - 生成训练数据：受掩码自编码器的启发，本工作通过随机遮蔽历史样例中的已知视觉变量，为自监督学习构建训练数据。
...
#slide(composer: (1fr, 1fr))[
  === #smallcaps("CubeBench")
  #text(0.9em)[
    将魔方状态接地评估为一个结构化视觉规划问题。每个实例渲染一个魔方状态；智能体必须重建足够的符号化的魔方小面信息，以供下游魔方求解器使用。每个视觉变量是一个小面，其值为六种颜色之一。
  ]
][
