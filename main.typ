...
#slide(composer: (1.2fr, 1.38fr))[
  #text(0.8em)[
    - 为突破 VLMs 在视觉规划中的感知限制，本工作将"以图思考"定义为一种从视觉证据中构建"规划充分"的符号化想象空间的方法 (2.1)
    - 为降低符号化想象空间构建成本，本工作引入一种新的"以图思考"策略——模式推断，从而使 VLMs 能够在规划任务中主动识别已知视觉模式，并直接推断局部符号化想象空间结构 (2.2)
    - 为通过学习获得这些可组合且可复用的模式，本工作提出模式归纳。这是一种从经验中在线构建模式库的归纳学习方法 (2.3)
  ]
][
  #figure(
...
#slide(composer: (1fr, 1fr))[
  ...
  #text(0.9em)[
    智能体观察一个渲染网格世界，并必须从起点到终点找到一条不坠入坑洞的最短安全路径。本工作将地图生成过程改成由基于 6 个模式的随机生成。每个视觉变量是一个网格单元，其值为单元类型
  ]
