...
#slide(composer: (1.2fr, 1.38fr))[
  ...
  - 虽然#text(green)["以图思考" (Thinking with Images, TWI) 这一新型范式，一定上缓解了上述的视觉感知瓶颈]；但是，#text(red)[即使当前 VLM 已经接受了较充分的通用 TWI 能力训练，它们在规划领域中的感知障碍仍然存在]
][
...
- PaLM-E: 初步将视觉观测和语言指令可以被整合起来，以支持序列机器人操作中的高层规划
- ReplanVLM: 引入闭环视觉反馈，从而能够检测执行失败并相应修正规划
- Reflective Planning: 通过想象未来世界状态的方式，改进长程操作规划
...
同时，与先前的面向规划"以图思考"不同，我们的工作将*此过程形式化为面向规划器充分性的符号化想象空间构建过程*。即，智能体必须决定图片的哪些部分与规划相关，以及当前符号化想象空间何时已经足以让真实规划器进行规划。
...
我们的工作利用 VLM *同时进行感知和生成符号模式提议*。同时，为了抑制错误的归纳结果，受混合专家模型和掩码自编码器的启发，我们的工作利用随机遮蔽的历史轨迹作为训练数据，以基于梯度的优化方式，调整不同专家的权重，降低错误或者无关专家的权重。
...
#slide(composer: (1.2fr, 1.38fr))[
  #text(0.7em)[
    - 为突破 VLMs 在视觉规划中的感知限制，本文将"以图思考"定义为一种从视觉证据中构建"规划充分"的符号化想象空间的方法 (2.1)
    - 为降低符号化想象空间构建成本，本文引入一种新的"以图思考"策略——模式推断，从而使 VLMs 能够在规划任务中主动识别已知视觉模式，并直接推断局部符号化想象空间结构 (2.2)
    - 为通过学习获得这些可组合且可复用的模式，本文提出模式归纳。这是一种从经验中在线构建模式库的归纳学习方法 (2.3)
  ]
][
  #figure(
    image("./figure/pipeline_cn.pdf", width: 110%),
    caption: [#smallcaps("PI-TWI") 方法概览],
  )
]
...
#slide(composer: (1.2fr, 1.38fr))[
  #text(0.7em)[
    ...
    - 一个未知的视觉变量值的概率分布，有两个作用
      ...
      - *重排序*：帮助决定下一步应揭示哪个变量，从而*将感知调用投入到更可能帮助规划器的变量*上
  ]
][
  #figure(
    image("./figure/qualitative_craft_cn.pdf", width: 110%),
...
#slide(composer: (1.2fr, 1.38fr))[
  #text(0.7em)[
    ...
    - *权重训练*
      - 生成训练数据：受掩码自编码器的启发，本文通过随机掩盖符号化想象空间中的符号事实，为自监督学习构建训练数据。
      - 基于梯度优化：对掩码处的视觉变量进行概率推断，以最大似然估计为优化目标，进行基于梯度优化
  ]
][
  #figure(
    image("./figure/qualitative_craft_cn.pdf", width: 110%),
...
#slide(composer: (1fr, 1fr))[
  ...
  #text(0.9em)[
    智能体观察一个渲染网格世界，并必须从起点到终点找到一条不坠入坑洞的最短安全路径。本文将地图生成过程改成由基于 6 个模式的随机生成。每个视觉变量是一个网格单元，其值为单元类型
  ]
...
#slide(composer: (1fr, 1fr))[
  === #smallcaps("Crafter")
  #text(0.9em)[
    #smallcaps("Crafter") 原本是一个随机生存环境。本文将其改造为一个确定性的视觉资源获取和合成任务：PI-TWI 智能体获得任务规格，并必须接地足够多的地图信息，以规划一系列可行的导航、收集和合成动作。每个视觉变量是一个网格单元，其值为对应的地形、物体或资源类型。
  ]
][
...
- 基线、消融实验和基准设置
  - 效率：本文使用*总 token 消耗*作为效率代理指标，其中包括*感知 token* 和*提议 token*。总 token 和感知 token 在所有回合上取平均，提议 token 在所有提议上取平均。
  - 准确性：*规划准确率*和*接地准确率*。
...
#slide(composer: (1.2fr, 1.38fr))[
  ...
  - 原生 TWI 能够#text(green)[改善部分环境-模型组合的表现]
...
#slide(composer: (1.2fr, 1.38fr))[
  *从总 token 消耗来看，本文方法 PI-TWI 在所有任务中都显著优于其他设置*
...
- 本文希望测试：在较小地图上完成训练后，模式及其权重能否零样本泛化到更大的地图。这意味着不仅模式固定，权重也固定，并且不会在更大地图上进行任何训练。
- 具体地：本文将 Crafter 在 64x64 地图上学习到的模式和权重测试到更大的 128x128 地图上。结果表明，模式及其权重*具有较强的分布外泛化能力*。主动揭示次数从 2349.63 下降到 1570.45，说明该方法具有有效性。
...
#slide[
  ...
  #text(0.7em)[
    - 作为一项概念验证研究，本文工作*聚焦于确定性视觉规划基准，而非真实世界或随机环境*
    - 本文还*假设可能的变量类型集合以及每个视觉变量的边界框均已提前给定*
      - 从这个意义上说，本文方法只解决了更广义 TWI 效率问题中的一部分：当每次揭示操作都返回正确视觉信息且符号接口已经指定时，如何降低接地成本。
  ]
